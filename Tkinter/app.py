...
def login(master):
        ...
        ttk.Button(login_frame, text="Login", command=lambda: getData(master, window, username = username.get(), password=password.get())).grid(column=1, row=2, pady=5)

def getData(master, window, **kwargs):
        if "user" in kwargs.values() and "admin" in kwargs.values():
            ...
            ttk.Label(main_frame, text="Students", font=("TkDefaultFont", 20)).grid(column=0, row=0)

            treeview = ttk.Treeview(main_frame, columns=("Student Id", "Idno", "Lastname", "Firstname", "Course", "Level"), show="headings")
